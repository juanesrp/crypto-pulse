...
async def create_alert(user_id: str, coin: str, threshold: float, db: AsyncSession):
    ...
    result = await db.execute(
        select(Alert).where(Alert.user_id == user_id, Alert.coin == coin)
    )
    ...
    if existing_alert:
        ...
        new_alert = Alert(user_id=user_id, coin=coin, threshold=threshold)
        db.add(new_alert)

    await db.commit()

    await redis_client.hset(f"{CACHE_PREFIX}{user_key}", coin, str(threshold))
...
async def get_alerts(user_id: str, db: AsyncSession):
    ...
    cached = await redis_client.hgetall(f"{CACHE_PREFIX}{user_key}")
